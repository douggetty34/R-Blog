...
Change
```
The function takes quite a while to run, so I ran it once and then saved the results in .csv to save myself 10 minutes every time I wanted to run this document. I left the function call as a comment in the code chunk above.
...
resample8 <- read.csv("resample8.csv")
```
...
With pilot sample of only 8 participants, there seems to be a great deal of variability around the effect size. Most effect sizes are clustered near the true effect size of `r Change$effect[59]` (blue line), though it would be plausible to estimate an effect size that's near zero or even negative. Obtaining an effect size near or less than zero would have a large influence on the results of a power analysis. This suggests to me that a pilot sample of this size might not be large enough. Let's see if the effect size estimates get more accurate with larger pilot samples. 
...
```{r}
resample8 <- read_csv("resample8.csv")
resample12 <- read_csv("resample12.csv")
resample16 <- read_csv("resample16.csv")
resample20 <- read_csv("resample20.csv")
